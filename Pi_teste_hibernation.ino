...
const long hibernation_duration_ms = 4 * 60 * 60 * 1000; // 4 hours in milliseconds
...
void setup() {
    ...
    // Set up the wakeup source for deep sleep
    esp_sleep_enable_timer_wakeup(hibernation_duration_ms * 1000); // Convert to microseconds
}
...
void loop() {
    ...
    readTurbid();
    readTDS();
    Serial.println("tds_var:");
    Serial.println(tds_var);
    Serial.println("turbid_var_ntu:");
    Serial.println(turbid_var_ntu);
    Serial.println("turbid_var_volt:");
    Serial.println(turbid_var_volt);
